...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for i, (L, ab) in enumerate(train_loader):
            ...
            # 感知损失 - 修复了 repeat(1, 1.5...) 的错误，改用 expand
            # 我们需要将 1通道L + 2通道ab 拼成 3通道喂给 VGG
            input_vgg_pred = torch.cat([L, ab_pred], dim=1)
            input_vgg_gt = torch.cat([L, ab], dim=1)

            # 如果是单通道输入扩充到3通道，使用 .expand(-1, 3, -1, -1)
        ...
        with torch.no_grad():
            ...
            # 转换并绘图 (取 Batch 第0张)
            gt_rgb = tensor2rgb(L_val[0], ab_val[0])
            pred_rgb = tensor2rgb(L_val[0], ab_pred_val[0])
            gray_img = (L_val[0].cpu().squeeze().numpy() + 1.0) / 2.0
